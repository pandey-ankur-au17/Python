...
def rent_flat(self):
    ...
    def rent_out(self):
        if self.current_renter==None:
            ...
            print(self.current_renter)
    rent_out(c)
    if self.has_elevator==True and self.fire_safety==True:
        self.rent=self.rent+500+500
        print("Upadated rent is:",self.rent)
    else:
        print("rent is same:",self.rent)
    
    for i in range(len(self.amneties)):
        self.rent=self.rent+500
    self.rent=self.maintainance_monthly+self.rent
    print("Your final Updated rent is:",self.rent)

rent_flat(c)

def issue_parking_spot(self):
    if self.parking_spots <= 1:
        self.parking_spot="Issued"
        print(self.parking_spot)
    else:
        print("No Parking is Available")

issue_parking_spot(c)



def revoke_parking_spot(self):
    self.bed_room=int(input("Enter flat number in 101 or 102 format:"))
    if self.has_parking_permission==False:
        self.has_parking_permission=True
        print("Parking Issued")
    else:
        print("revoking failed")

revoke_parking_spot(c)